...
class ActiveTradeManager:
    ...
    async def manage_open_trades(self) -> None:
        ...
        for pos in positions:
            magic = getattr(pos, 'magic', None) or (isinstance(pos, dict) and pos.get('magic'))
    ...
    async def _apply_trade_management(self, pos: TradePosition, symbol_info: SymbolInfo) -> None:
        try:
            ...
            # 1. Breakeven Check
            if self.enable_breakeven and current_profit_pips >= self.breakeven_trigger_pips:
                new_sl = pos.openPrice + (self.breakeven_lock_pips * pip_size) if is_buy else pos.openPrice - (self.breakeven_lock_pips * pip_size)
                ...
                if is_buy and (pos.stopLoss is None or pos.stopLoss < new_sl):
                    needs_update = True
                elif not is_buy and (pos.stopLoss is None or pos.stopLoss > new_sl or pos.stopLoss == 0):
                    needs_update = True
                    
                if needs_update:
                    logger.info(f"Moving SL to breakeven for {pos.symbol} (ID: {pos.id})")
                    await self.broker.modify_position(pos.id, stop_loss=new_sl, take_profit=pos.takeProfit)
                    return # Skip trailing stop logic this cycle
                    
            # 2. Trailing Stop Check
            if self.enable_trailing_stop and current_profit_pips >= self.trailing_stop_pips:
                # Calculate new trailing stop loss
                trail_distance = self.trailing_stop_pips * pip_size
                new_sl = pos.currentPrice - trail_distance if is_buy else pos.currentPrice + trail_distance
                
                needs_update = False
                if is_buy and (pos.stopLoss is None or new_sl > pos.stopLoss):
                    needs_update = True
                elif not is_buy and (pos.stopLoss is None or new_sl < pos.stopLoss or pos.stopLoss == 0):
                    needs_update = True
